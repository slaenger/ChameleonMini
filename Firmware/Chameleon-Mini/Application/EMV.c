...
// Catch DataTransmission from PC
...
static uint16_t ProtocolProcess(uint8_t* const Buffer, uint16_t ByteCount)
{
    ...
    if (ISO14443ACheckCRCA(Buffer, ByteCount - ISO14443A_CRCA_SIZE)&& ByteCount >=3) // Block should be Appended with CRCA check this
    {
        ...
        case 2 : // R Block
        {
            ...
            if (DataChaining)
            {
                // We are in DataChaining Mode so check Blocknumber and report back to PC
    ...
    else
    {
        ...
        case STATE_PROTOCOL:
        {
            ...
            if (ByteCount <3 && ISO14443AWakeUp(Buffer, &BitCount, ATQA_VALUE, FromHalt)){
                // We received a frame that is too small to be handed over ( Transmission of ATS probably failed) so check if wakeupcall if it is we go back to STATE_HALT
                // This does serve to reduce time until retry
                State = FromHalt ? STATE_HALT : STATE_IDLE;
                return BitCount;
            }
